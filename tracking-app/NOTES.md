...
- [ ] **Stage 2: Accounts + login** — D1 user schema, WebCrypto PBKDF2 hashing, JWT via cookie, auth middleware.
- [ ] **Stage 3: Expense CRUD** — D1 expenses schema, integer cents validation, unified JSON error shape, curl verified.
- [ ] **Stage 4: Usable UI** — `hono/jsx` SSR pages, expense list, add/edit modals/forms, delete confirmation, empty state.
- [ ] **Stage 5: Tags + filters** — Many-to-many / tag column, date range & tag filtering in D1 SQL.
